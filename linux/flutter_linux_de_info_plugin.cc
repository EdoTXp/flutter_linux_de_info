...
#include "flutter_linux_de_info_plugin_private.h"

#define FLUTTER_LINUX_DE_INFO_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), flutter_linux_de_info_plugin_get_type(), \
                              FlutterLinuxDeInfoPlugin))
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  FlutterLinuxDeInfoPlugin* plugin = FLUTTER_LINUX_DE_INFO_PLUGIN(user_data);
  flutter_linux_de_info_plugin_handle_method_call(plugin, method_call);
}

void flutter_linux_de_info_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
